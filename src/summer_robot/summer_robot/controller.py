...
class Nav2Controller:
    ...
    def _on_goal_response(self, future):
        ...
        if not goal_handle.accepted:
            self.node.get_logger().warn(f"[Exlpore] Refuse target: {self.current_goal}")
    ...
    def _on_navigation_finished(self, future):
        ...
        if success:
            ...
            self.node.get_logger().warn(f"[Explore] Navigation unsucces (Status: {status})，Noted to unreachable")
